...
Why didn't the challenge author just parse the string which needed to be evaluated directly into the `eval()`function, instead of declaring a global variable `_EXEC_CMD` then parsing it? Maybe we can overwrite the value of this variable to conduct RCE.
